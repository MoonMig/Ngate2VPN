...
### Добавлено
- **Русский и английский язык интерфейса.** По умолчанию язык берётся из системы (русский, если он первый из поддерживаемых в настройках macOS, иначе английский); в Settings → Interface → **Language** можно принудительно выбрать System / English / Русский. Переводятся вкладки, экраны, редактор профиля, журнал (заголовки и кнопки), меню в трее, меню правого клика, окна-предупреждения, тексты ошибок и справка. Смена языка применяется сразу, без перезапуска. Записи `[SYSTEM]` в журнале и вывод ngate-client намеренно остаются на английском (это диагностика, она пишется в файлы и ищется по точным формулировкам). Стандартные пункты системного меню (About, Hide, Quit и т. п.) следуют языку системы и меняются только при перезапуске.
- Тесты: `LocalizationTests` (перевод, форматные подстановки, покрытие ошибок и состояний туннелей); всего 71 тест.

### Изменено
- Подписи полей («Название», «Бинарник») больше не переносятся на вторую строку в русском интерфейсе: колонка подписей стала шире.
- Справка обновлена под текущие возможности (прогрев, 2FA, меню в трее, язык).
- Из вкладки «Настройки» убраны два поясняющих текста внизу (про отключение туннелей при закрытии и про хранение секретов).
- Из строки меню приложения убраны пункты «Правка», «Вид» и «Окно» (в том числе пока меню ещё пустые — опознаются и по названию), меню «Справка» тоже убрано из строки меню, а справка перенесена в меню приложения («Ngate2VPN» → «Help» / «Помощь», Cmd+?) рядом с «Настройки…» и добавлена в меню в трее над «Выйти». Пункты меню приложения теперь переключаются вместе с языком без перезапуска. Cmd+C / V / X / A / Z продолжают работать: сочетания обрабатываются напрямую (по кодам клавиш, поэтому и на русской раскладке).
